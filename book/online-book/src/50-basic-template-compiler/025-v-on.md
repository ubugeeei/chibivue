...
さて、変換処理を実装していくのですが、実はまだその変換後の Node を表す AST がありません。  
と言うのも、今回は元 Node を使いつつ、周辺の文字列を結合したいはずです。以下のようなイメージです。  
※ exp は元の式です。(文字列ではありません。)

```ts
exp = ["$event => {", exp, "}"];
```

exp から content(ソースコードの文字列)を取り出して、文字列結合して、それをパースして JSFunction の AST に変換するのはあまりに非効率です。

そこで、Vue ではこのような複合的な Node を表す AST を導入しています。  
それが、`CompoundExpression` です。  
...
実際に、children として `["$event => {", exp, "}"]` のような配列を取ることができます。  
codegen する際は、文字列の場合はそのまま generate し、そのほかはいつも通り Node からソースコードを generate します。
結構ヘンテコな感じです。children が string の型をとっているのはこのような形になるためです。

この AST を利用して、今回の変換処理は以下のようにして実装することができます。
ついでに、セミコロンの有無で複数の分があるかどうかを判定し、paren を使うか brace を使うか判定してます。
...
ここまでのソースコード:  
[GitHub](https://github.com/Ubugeeei/chibivue/tree/main/book/impls/50_basic_template_compiler/025_v_on)

### イベント修飾子

Coming soon...  
https://github.com/Ubugeeei/chibivue/issues/108